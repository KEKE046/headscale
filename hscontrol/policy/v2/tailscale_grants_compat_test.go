...
func testGrantSuccess(
	...
	for nodeName, capture := range tf.Captures {
		t.Run(nodeName, func(t *testing.T) {
			...
			// Strip the relay CapGrant rule that is always appended by this
			// fork; compat tests compare against upstream Tailscale output
			// which does not include the automatic relay rule.
			gotRules = stripRelayCapGrantRule(gotRules)
...
func stripRelayCapGrantRule(rules []tailcfg.FilterRule) []tailcfg.FilterRule {
	if len(rules) == 0 {
		return rules
	}
	...
	for i := len(rules) - 1; i >= 0; i-- {
		r := rules[i]
		if len(r.CapGrant) > 0 && len(r.DstPorts) == 0 && len(r.SrcIPs) == 1 && r.SrcIPs[0] == "*" {
